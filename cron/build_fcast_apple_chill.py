...
temps_last_obs = reader.dateAttribute('temps.maxt', 'last_obs_date', None)
if temps_last_obs is not None:
    max_last_obs = max(chill_last_obs, temps_last_obs)
    min_last_obs = min(chill_last_obs, temps_last_obs)
else: min_last_obs = max_last_obs = chill_last_obs
if min_last_obs > season_end:
...
date_attrs = { 'last_obs_date': max_last_obs.strftime('%Y-%m-%d'),
...
start_chill = min_last_obs + ONE_DAY
