...
class ProcessServer(object):
    ...
    def __init__(self, varieties, target_year, reporter, download_temps,
                       build_grids, draw_maps, publish_maps, debug=False,
                       test_run=False):
...
parser.add_option('-t', action='store_false', dest='download_temps',
                  default=True)
parser.add_option('-v', action='store', dest='varieties',
                  default=APPLE.varieties.build)
...
if isinstance(varieties, basestring):
    if ',' in varieties:
        varieties = tuple(varieties.split(','))
    else: varieties = (varieties,)
...
process_server = ProcessServer(varieties, target_year, reporter, download_temps,
