...
for model_name in models:
    ...
    manager.open('r')
    daily_chill, accumulated_chill = manager.estimateChill(model_name, **chill_kwargs)
    ...
    if update_db:
        ...
        manager.open('a')
        manager.updateChill(model_name, daily_chill, accumulated_chill, start_date)
        last_valid = manager.getDatasetAttribute('%s.daily' % model, 'last_valid_date')
        manager.close()

        last_valid = datetime.date(*tuple([int(d) for d in last_valid.split('-')]))
        if last_valid > max_valid_date:
            manager.open('a')
            manager.setDatasetAttribute('%s.daily' % model, 'last_valid_date', max_valid_str)
            manager.setDatasetAttribute('%s.accumulated' % model, 'last_valid_date', max_valid_str)
            manager.setDatasetAttribute('%s.provenance' % model, 'last_valid_date', max_valid_str)
            manager.close()
    ...
    if calc_gdd:
        ...
        for low_threshold, high_threshold in gdd_thresholds:
            ...
            if update_db:
                group_path = manager.gddGroupName(low_threshold, high_threshold)
                if not manager.hasGroup(group_path):
                    manager.open('a')
                    manager._initGddDatasets(low_threshold, high_threshold)
                    manager.close()
                manager.open('a')
                manager.updateGdd(low_threshold, high_threshold, daily_gdd, start_date)
                last_valid = manager.getDatasetAttribute('%s.daily' % group_path, 'last_valid_date')
                manager.close()

                last_valid = datetime.date(*tuple([int(d) for d in last_valid.split('-')]))
                if last_valid > max_valid_date:
                    manager.open('a')
                    manager.setDatasetAttribute('%s.daily' % group_path, 'last_valid_date', max_valid_str)
                    manager.setDatasetAttribute('%s.provenance' % group_path, 'last_valid_date', max_valid_str)
                    manager.close()
