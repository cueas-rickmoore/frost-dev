...
def temp3DGridToHourly(dates, lats, mint, maxt, units='F', debug=False):
    if units == 'F':
        _mint = (mint - 32) * 5 / 9
        _maxt = (maxt - 32) * 5 / 9
    else:
    ...
    for indx, date in enumerate(dates):
        hourly = tempGridToHourly(date, lats, _mint[indx], _maxt[indx], 'C',
                                  debug=debug)
        hourly_array.append(hourly)
    return N.array(hourly_array, dtype=float)


# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #

def tempGridToHourly(date, lats, mint, maxt, units='F', debug=False):
    ...
    if date.month >= 3:
        clim_day = int(30.6 * date.month + date.day - 91.3)
    else: clim_day = int(30.6 * (date.month+12) + date.day - 91.3)
